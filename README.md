...
这是一个可复用的 Harness 工作流部署 Skill，已经同时适配 Codex 和 Claude Code 的插件分发结构。
...
- 把 Harness creator skill 打包成 Codex 或 Claude Code 可安装插件。
...
```bash
python3 skills/harness-framework-deployer/scripts/check_deployed_harness.py . --mode plugin --plugin-runtime both
```
...
- 更新插件结构后，需要同时跑 Codex 和 Claude Code 校验。
